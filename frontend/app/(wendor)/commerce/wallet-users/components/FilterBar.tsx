...
import { WalletUserFilterOptions, WalletUserQuery } from '@/types/wallet-user';

interface Props {
  query: WalletUserQuery;
  options: WalletUserFilterOptions | null;
  onChange: (patch: Partial<WalletUserQuery>) => void;
  onReset: () => void;
}
...
export function FilterBar({ query, options, onChange, onReset }: Props) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-console border border-line bg-panel p-3">
      <input
        type="text"
        placeholder="Search by name, RFID or phone..."
        value={query.search ?? ''}
